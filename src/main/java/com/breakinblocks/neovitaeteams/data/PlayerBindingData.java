package com.breakinblocks.neovitaeteams.data;
...
public class PlayerBindingData {
    private static final String NBT_KEY = "NVTeamsData";
    ...
    private static void saveToNBT(ServerPlayer player, BindingPreference pref) {
        CompoundTag persistentData = player.getPersistentData();
        CompoundTag nvData = new CompoundTag();

        nvData.putString(BINDING_MODE_KEY, pref.mode.name());
        nvData.putBoolean(DONT_ASK_KEY, pref.dontAsk);
        if (pref.targetTeamId != null) {
            nvData.putUUID(TARGET_TEAM_KEY, pref.targetTeamId);
        }

        persistentData.put(NBT_KEY, nvData);
    }
    ...
    private static BindingPreference loadFromNBT(Player player) {
        ...
        CompoundTag nvData = persistentData.getCompound(NBT_KEY);

        try {
            BindingMode mode = BindingMode.valueOf(nvData.getString(BINDING_MODE_KEY));
            boolean dontAsk = nvData.getBoolean(DONT_ASK_KEY);
            UUID targetTeam = nvData.contains(TARGET_TEAM_KEY) ? nvData.getUUID(TARGET_TEAM_KEY) : null;
